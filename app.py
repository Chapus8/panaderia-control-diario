...
if not st.session_state['logueado']:
    ...
    with col2:
        ...
        else: st.markdown("<h1 style='text-align: center;'>🍞 Panadería Judith</h1>", unsafe_allow_html=True)
...
# ------------------------------------------
# MÓDULO 8: PROVEEDORES (CON PAGOS Y DEUDAS PAGADAS)
# ------------------------------------------
    ...
    try:
        ...
        with tab_prov3:
            ...
            if not deudas_activas.empty:
                ...
                st.write("Selecciona una deuda de la lista de arriba e ingresa cuánto vas a pagar. Si el saldo llega a Q 0.00, la deuda pasará al historial de pagadas.")
                
                opciones_deuda = deudas_activas.apply(lambda row: f"ID: {row['id']} | {row['proveedor']} - Doc: {row['documento']} | Saldo: Q {row['saldo']}", axis=1).tolist()
                deuda_sel = st.selectbox("Selecciona la deuda a abonar/pagar:", opciones_deuda)
                
                col_p1, col_p2 = st.columns(2)
                monto_abono = col_p1.number_input("Monto a Pagar/Abonar (Q)", min_value=0.01, step=50.00)
                
                if col_p2.button("✅ Aplicar Pago", use_container_width=True):
                    idx_deuda = opciones_deuda.index(deuda_sel)
                    id_deuda = int(deudas_activas.iloc[idx_deuda]['id'])
                    saldo_actual = float(deudas_activas.iloc[idx_deuda]['saldo'])
                    
                    if monto_abono > saldo_actual:
                        st.warning(f"⚠️ El monto a pagar (Q {monto_abono}) no puede ser mayor al saldo pendiente (Q {saldo_actual}).")
                    else:
                        nuevo_saldo = saldo_actual - monto_abono
                        estado_nuevo = 'Pagado' if nuevo_saldo <= 0 else 'Pendiente'
                        
                        with conn.session as s:
                            s.execute(text("UPDATE cuentas_por_pagar SET saldo_pendiente = :ns, estado = :est WHERE id = :id"), {"ns": nuevo_saldo, "est": estado_nuevo, "id": id_deuda})
                            s.commit()
                        st.success(f"✅ Pago aplicado correctamente. Nuevo saldo: Q {nuevo_saldo:.2f}")
                        st.rerun()
            else:
                st.info("No tienes deudas pendientes registradas en este momento. ¡Felicidades!")
        ...
        with tab_prov4:
            ...
            if not deudas_pagadas.empty:
                st.dataframe(deudas_pagadas, column_config={"vencimiento": st.column_config.DateColumn("Vencimiento", format="DD/MM/YYYY"), "total": st.column_config.NumberColumn("Total Pagado", format="Q %.2f")}, use_container_width=True, hide_index=True)
            else:
                st.info("Aún no tienes deudas pagadas en el historial.")
                
    except Exception as e: st.error(f"Error: {e}")

# ------------------------------------------
# (EL RESTO DE MÓDULOS DE REGISTRO, HISTORIAL, ESTADÍSTICAS, COMPARATIVA, DÍAS ESTRELLA, RUTA XML, VENTAS EXTRA, PLANILLAS Y REPORTE MENSUAL SE MANTIENEN EXACTAMENTE IGUAL)
# ------------------------------------------
