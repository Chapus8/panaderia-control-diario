...
# -- FUNCIONES PDF (Compactadas para eficiencia) --
...
# ------------------------------------------
    ...
    try:
        ...
        if not corte_data.empty:
            ...
            sum_efectivo = sum_venta + sum_pedidos; total_ingresos_bruto_hist = sum_efectivo + sum_transferencias; sum_gastos = gastos_hist['monto'].sum() if not gastos_hist.empty else 0.0; neto_efectivo = sum_efectivo - sum_gastos
                    ...
                    if st.form_submit_button("💾 Guardar Corrección"):
                        with conn.session as s:
                            ...
                            if existe: s.execute(text("UPDATE ingresos SET venta_total = :v, credito_pagado = :p, transferencias = :t WHERE corte_id = :cid"), {"v": nuevo_efectivo, "p": nuevo_pedidos, "t": nuevo_trans, "cid": int(corte_id)})
                                ...
                                s.execute(text("INSERT INTO ingresos (corte_id, ruta_id, venta_total, credito_pagado, transferencias) VALUES (:c, :r, :v, :cp, :t)"), {"c": int(corte_id), "r": ruta_default, "v": nuevo_efectivo, "cp": nuevo_pedidos, "t": nuevo_trans})
                            s.commit()
        ...
        with st.spinner("Cargando los datos día por día..."):
            try:
                ...
                if df_ing.empty and df_gas.empty: st.warning("No hay registros en esas fechas.")
                    ...
                    df_resumen = df_resumen.sort_values('fecha'); df_resumen['utilidad'] = df_resumen['ingresos'] - df_resumen['gastos']
                    t_ing = df_resumen['ingresos'].sum(); t_gas = df_resumen['gastos'].sum(); t_uti = df_resumen['utilidad'].sum()
                    st.markdown("---"); c1, c2, c3 = st.columns(3)
                    c1.metric("💰 Total Ingresos del Rango", f"Q {t_ing:,.2f}"); c2.metric("📉 Total Gastos del Rango", f"Q {t_gas:,.2f}"); c3.metric("⚖️ Utilidad del Rango", f"Q {t_uti:,.2f}"); st.markdown("---")
...
# ------------------------------------------
# MÓDULO 6: RUTA Y PEDIDOS (XML) LECTOR NATIVO DE SAT
# ------------------------------------------
elif opcion_menu == "🚚 Ruta y Pedidos (XML)":
    st.title("🚚 Control de Ruta y Pedidos (Lector SAT Nativo)")
    ...
    with tab_xml:
        ...
        if archivo_xml is not None:
            try:
                # Lector nativo inteligente: ignora namespaces sin destruir el archivo
                ...
                # Buscar datos generales ignorando prefijos de SAT
                ...
                if es_oasis:
                    opciones_sucursal = ["Oasis Chiquimula", "Oasis Esquipulas", "Oasis Quezaltepeque", "Oasis Ipala", "Oasis Zacapa", "Qualy Central", "Otra Sucursal Oasis"]
                ...
                lista_items = []
                # Buscar items con lector inteligente
                for elem in root.iter():
    ...
    with tab_estadisticas_rutas:
        ...
        try:
            ...
            else: st.info("Sube facturas para ver las estadísticas de tus sucursales.")
